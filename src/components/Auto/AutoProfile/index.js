        ...
        <Descriptions column={1}>
          {
            columnList.filter(column => (column.profileDisplayFlag === 'Y')).map(column => (
              <Item key={column.columnCode}
                    label={<strong>{column.columnName}</strong>}><span>{this.getDescription(column, data)}</span></Item>
            ))
          }
        </Descriptions>
      );
        ...
        <ButtonGroup>
          {actionList.filter(action => (action.type === 'L')).map(action => {
            ...
            if (action.propFlag === 'Y') {
              ...
              return (
                <Popconfirm key={action.code} placement="top" title={propMsg} onConfirm={() => {
                  if (fn) {
                    fn(data);
                  }
                }} okText="确定" cancelText="取消">
                  <Button disabled={this.handleRowOperation(data, action)} type={action.displayType}
                          icon={action.icon}>{action.name || '操作'}</Button>
                </Popconfirm>
              );
            }
            return (
              <Button key={action.code} onClick={() => (fn && fn(data))} type={action.displayType}
                      icon={action.icon}>{action.name || '操作'}</Button>
            );
