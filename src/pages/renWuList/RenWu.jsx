...
export default class RenWu extends PureComponent {
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="人物名称" name="renWuName">
            <Input placeholder="请输入人物名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="人物描述" name="renWuMiaoShu">
            <Input placeholder="请输入人物描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
    ...
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            renWuName: profile.renWuName,
            renWuCode: profile.renWuCode,
            xiaoShuoId: profile.xiaoShuoId,
            renWuMiaoShu: profile.renWuMiaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, renWu: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
