...
import styles from './JiGou.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const jiGouColumns = [
  { columnName: '机构代码', columnCode: 'jiGouCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '机构名称', columnCode: 'jiGouName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '机构分类', columnCode: 'jiGouFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '机构描述', columnCode: 'jiGouMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ jiGou, xiaoShuo, loading }) => ({
  jiGou,
  xiaoShuo,
  loading: loading.models.jiGou,
}))
export default class JiGou extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = jiGouColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'jiGouMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, jiGou: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'jiGou/query',
      payload: { ...pagination, ...params },
    });
    const { jiGou: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'jiGou/emptyProfile',
    });
  ...
  handleData = () => {
    const { jiGou: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { jiGou: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { jiGou: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑机构"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { jiGou: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { jiGou: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="机构信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
