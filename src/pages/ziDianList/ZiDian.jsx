...
export default class ZiDian extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = ziDianColumns.filter(column => {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, ziDian: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'ziDian/query',
      payload: { ...pagination, ...params },
    });
    const { ziDian: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'ziDian/emptyProfile',
    });
  ...
  handleData = () => {
    const { ziDian: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { ziDian: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { ziDian: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { ziDian: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="字典信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
