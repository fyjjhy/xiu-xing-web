...
let columns = [{
  title: '妖兽代码',
  dataIndex: 'yaoShouCode',
  width: '90px',
}, {
  title: '妖兽名称',
  dataIndex: 'yaoShouName',
  width: '175px',
}, {
  title: '小说',
...
export default class YaoShou extends PureComponent {
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="妖兽名称" name="yaoShouName">
            <Input placeholder="请输入妖兽名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="妖兽描述" name="yaoShouMiaoShu">
            <Input placeholder="请输入妖兽描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
    ...
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            yaoShouName: profile.yaoShouName,
            yaoShouCode: profile.yaoShouCode,
            xiaoShuoId: profile.xiaoShuoId,
            yaoShouMiaoShu: profile.yaoShouMiaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, yaoShou: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
