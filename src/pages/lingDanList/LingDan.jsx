...
import styles from './LingDan.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const lingDanColumns = [
  { columnName: '灵丹代码', columnCode: 'lingDanCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '灵丹名称', columnCode: 'lingDanName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '灵丹分类', columnCode: 'lingDanFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '灵丹描述', columnCode: 'lingDanMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ lingDan, xiaoShuo, loading }) => ({
  lingDan,
  xiaoShuo,
  loading: loading.models.lingDan,
}))
export default class LingDan extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = lingDanColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'lingDanMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, lingDan: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'lingDan/query',
      payload: { ...pagination, ...params },
    });
    const { lingDan: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'lingDan/emptyProfile',
    });
  ...
  handleData = () => {
    const { lingDan: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { lingDan: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { lingDan: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑灵丹"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { lingDan: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { lingDan: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="灵丹信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
