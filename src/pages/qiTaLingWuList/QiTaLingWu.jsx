...
let columns = [{
  title: '灵物代码',
  dataIndex: 'lingWuCode',
  width: '90px',
}, {
  title: '灵物名称',
  dataIndex: 'lingWuName',
  width: '175px',
}, {
  title: '小说',
...
@connect(({ lingWu, xiaoShuo, loading }) => ({
  lingWu,
  xiaoShuo,
  loading: loading.models.lingWu,
}))
export default class QiTaLingWu extends PureComponent {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, lingWu: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'lingWu/query',
      payload: { ...pagination, ...params },
    });
    const { lingWu: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'lingWu/emptyProfile',
    });
  ...
  handleData = () => {
    const { lingWu: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="灵物名称" name="lingWuName">
            <Input placeholder="请输入灵物名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="灵物描述" name="lingWuMiaoShu">
            <Input placeholder="请输入灵物描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { lingWu: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    const { currentModel } = this.state;
    const { lingWu: { data: selectRecord  } } = this.props;
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            lingWuName: profile.lingWuName,
            lingWuCode: profile.lingWuCode,
            xiaoShuoId: profile.xiaoShuoId,
            lingWuMiaoShu: profile.lingWuMiaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, lingWu: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { lingWu: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { lingWu: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="灵物信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
