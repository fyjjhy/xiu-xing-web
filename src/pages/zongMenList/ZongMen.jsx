...
const zongMenColumns = [
  { columnName: '宗门代码', columnCode: 'zongMenCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '宗门名称', columnCode: 'zongMenName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '宗门分类', columnCode: 'zongMenFenLei', valueType: 'S', displayType: 'S', valueList: `constant|${JSON.stringify(zongMenFenLeiConstant())}`,hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '宗门描述', columnCode: 'zongMenMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ zongMen, xiaoShuo, loading }) => ({
  zongMen,
  xiaoShuo,
  loading: loading.models.zongMen,
}))
export default class ZongMen extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = zongMenColumns.filter(column => {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, zongMen: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'zongMen/query',
      payload: { ...pagination, ...params },
    });
    const { zongMen: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'zongMen/emptyProfile',
    });
  ...
  handleData = () => {
    const { zongMen: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { zongMen: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { zongMen: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑宗门"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { zongMen: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { zongMen: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="宗门信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
