...
let columns = [{
  title: '宗门代码',
  dataIndex: 'zongMenCode',
  width: '90px',
}, {
  title: '宗门名称',
  dataIndex: 'zongMenName',
  width: '175px',
}, {
  title: '宗门分类',
  dataIndex: 'zongMenFenLei',
  width: '100px',
}, {
  title: '小说',
...
const profileColumns = [{
  ...
  addField: 'Y',
  editField: 'Y',
  valueList: `constant|${JSON.stringify(zongMenFenLei)}`,
  displayType: 'S',
}, {
  columnName: '小说',
...
export default class ZongMen extends PureComponent {
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 6, lg: 24, xl: 48 }}>
        <Col md={6} sm={24}>
          <FormItem label="宗门名称" name="zongMenName">
            <Input placeholder="请输入宗门名称" />
          </FormItem>
        </Col>
        <Col md={6} sm={24}>
          <FormItem label="宗门描述" name="zongMenMiaoShu">
            <Input placeholder="请输入宗门描述" />
          </FormItem>
        </Col>
        <Col md={6} sm={24}>
          <FormItem label="宗门分类" name="zongMenFenLei">
            <Select placeholder="请选择宗门分类" allowClear>
              {zongMenFenLei.map(fenLei => {
    ...
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            zongMenName: profile.zongMenName,
            zongMenCode: profile.zongMenCode,
            zongMenFenLei: profile.zongMenFenLei,
            xiaoShuoId: profile.xiaoShuoId,
            zongMenMiaoShu: profile.zongMenMiaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, zongMen: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
