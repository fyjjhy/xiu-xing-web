...
import styles from './SheDing.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const sheDingColumns = [
  // { columnName: '设定代码', columnCode: 'sheDingCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '设定名称', columnCode: 'sheDingName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '设定分类', columnCode: 'sheDingFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '设定描述', columnCode: 'sheDingMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ sheDing, xiaoShuo, loading }) => ({
  sheDing,
  xiaoShuo,
  loading: loading.models.sheDing,
}))
export default class SheDing extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = sheDingColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'sheDingMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, sheDing: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'sheDing/query',
      payload: { ...pagination, ...params },
    });
    const { sheDing: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'sheDing/emptyProfile',
    });
  ...
  handleData = () => {
    const { sheDing: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { sheDing: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { sheDing: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑设定"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { sheDing: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { sheDing: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="设定信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
