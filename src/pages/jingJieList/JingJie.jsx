...
export default class JingJie extends PureComponent {
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="境界名称" name="name">
            <Input placeholder="请输入境界名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="境界描述" name="miaoShu">
            <Input placeholder="请输入境界描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
    ...
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            name: profile.name,
            jingJieCode: profile.jingJieCode,
            xiaoShuoId: profile.xiaoShuoId,
            miaoShu: profile.miaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, jingJie: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
