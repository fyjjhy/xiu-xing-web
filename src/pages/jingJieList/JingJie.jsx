...
import styles from './JingJie.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const jingJieColumns = [
  { columnName: '境界代码', columnCode: 'code', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '境界名称', columnCode: 'name', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '境界分类', columnCode: 'jingJieFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '境界描述', columnCode: 'miaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ jingJie, xiaoShuo, loading }) => ({
  jingJie,
  xiaoShuo,
  loading: loading.models.jingJie,
}))
export default class JingJie extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = jingJieColumns.filter(column => {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, jingJie: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'jingJie/query',
      payload: { ...pagination, ...params },
    });
    const { jingJie: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'jingJie/emptyProfile',
    });
  ...
  handleData = () => {
    const { jingJie: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { jingJie: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { jingJie: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑境界"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { jingJie: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { jingJie: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="境界信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
