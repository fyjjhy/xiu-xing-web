...
import styles from './LingCai.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const lingCaiColumns = [
  { columnName: '灵材代码', columnCode: 'lingCaiCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '灵材名称', columnCode: 'lingCaiName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '灵材分类', columnCode: 'lingCaiFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '灵材描述', columnCode: 'lingCaiMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ lingCai, xiaoShuo, loading }) => ({
  lingCai,
  xiaoShuo,
  loading: loading.models.lingCai,
}))
export default class LingCai extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = lingCaiColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'lingCaiMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, lingCai: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'lingCai/query',
      payload: { ...pagination, ...params },
    });
    const { lingCai: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'lingCai/emptyProfile',
    });
  ...
  handleData = () => {
    const { lingCai: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { lingCai: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { lingCai: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑灵材"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { lingCai: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { lingCai: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="灵材信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
