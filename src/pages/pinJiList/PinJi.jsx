...
@connect(({ pinJi, xiaoShuo, loading }) => ({
  pinJi,
  xiaoShuo,
  loading: loading.models.pinJi,
}))
export default class PinJi extends PureComponent {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, pinJi: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'pinJi/query',
      payload: { ...pagination, ...params },
    });
    const { pinJi: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'pinJi/emptyProfile',
    });
  ...
  handleData = () => {
    const { pinJi: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="品级名称" name="name">
            <Input placeholder="请输入品级名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="品级描述" name="miaoShu">
            <Input placeholder="请输入品级描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { pinJi: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    const { currentModel } = this.state;
    const { pinJi: { data: selectRecord  } } = this.props;
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, pinJi: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { pinJi: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { pinJi: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="品级信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
