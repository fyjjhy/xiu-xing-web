...
@connect(({ xiaoShuo, loading }) => ({
  xiaoShuo,
  loading: loading.models.xiaoShuo,
}))
export default class XiaoShuo extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = xiaoShuoColumns.filter(column => {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, xiaoShuo: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'xiaoShuo/query',
      payload: { ...pagination, ...params },
    });
    const { xiaoShuo: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'xiaoShuo/emptyProfile',
    });
  ...
  handleData = () => {
    const { xiaoShuo: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { xiaoShuo: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { xiaoShuo: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑小说"
  ...
  renderEditDialog() {
    ...
    if (currentModel === 'listEdit') {
      const { xiaoShuo: { data  } } = this.props;
      return (
        <StandardForm
          formColumnList={this.editColumns}
          currentModel={currentModel}
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { xiaoShuo: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { xiaoShuo: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="小说信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
