...
let columns = [{
  title: '地名代码',
  dataIndex: 'diMingCode',
  width: '90px',
}, {
  title: '地名名称',
  dataIndex: 'diMingName',
  width: '175px',
}, {
  title: '小说',
...
export default class DiMing extends PureComponent {
  ...
  renderSimpleForm() {
    ...
    return (
      <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
        <Col md={8} sm={24}>
          <FormItem label="地名名称" name="diMingName">
            <Input placeholder="请输入地名名称" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
          <FormItem label="地名描述" name="diMingMiaoShu">
            <Input placeholder="请输入地名描述" />
          </FormItem>
        </Col>
        <Col md={8} sm={24}>
    ...
    if (currentModel === 'edit') {
      ...
      return (
        <StandardForm
          ...
          initialValues={{
            diMingName: profile.diMingName,
            diMingCode: profile.diMingCode,
            xiaoShuoId: profile.xiaoShuoId,
            diMingMiaoShu: profile.diMingMiaoShu,
            id: profile.id
          }}
          // showDialog
          // visible
  ...
  renderAddDialog() {
    const { currentModel } = this.state;
    if (currentModel === 'add') {
      // const { xiaoShuo: { xiaoShuoList }, diMing: { data  } } = this.props;
      // const profile = data || {};
      return (
        <StandardForm
          formColumnList={profileColumns}
