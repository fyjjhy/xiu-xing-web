...
@connect(({ xiuXingRiZhi, xiaoShuo, loading }) => ({
  xiuXingRiZhi,
  xiaoShuo,
...
export default class XiuXingRiZhi extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = xiuXingRiZhiColumns.filter(column => {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, xiuXingRiZhi: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'xiuXingRiZhi/query',
      payload: { ...pagination, ...params },
    });
    const { xiuXingRiZhi: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
  ...
  handleData = () => {
    const { xiuXingRiZhi: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { xiuXingRiZhi: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { xiuXingRiZhi: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑修行日志"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { xiuXingRiZhi: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { xiuXingRiZhi: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="修行日志信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
