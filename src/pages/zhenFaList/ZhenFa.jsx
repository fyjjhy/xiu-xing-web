...
import styles from './ZhenFa.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const zhenFaColumns = [
  { columnName: '阵法代码', columnCode: 'zhenFaCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '阵法名称', columnCode: 'zhenFaName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '阵法分类', columnCode: 'zhenFaFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '阵法描述', columnCode: 'zhenFaMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ zhenFa, xiaoShuo, loading }) => ({
  zhenFa,
  xiaoShuo,
  loading: loading.models.zhenFa,
}))
export default class ZhenFa extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = zhenFaColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'zhenFaMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, zhenFa: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'zhenFa/query',
      payload: { ...pagination, ...params },
    });
    const { zhenFa: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'zhenFa/emptyProfile',
    });
  ...
  handleData = () => {
    const { zhenFa: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { zhenFa: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { zhenFa: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑阵法"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { zhenFa: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { zhenFa: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="阵法信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
