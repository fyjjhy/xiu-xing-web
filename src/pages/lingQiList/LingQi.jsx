...
import styles from './LingQi.less';
import {renderMiaoShu} from "../../utils/utils";

const { Group: ButtonGroup } = Button;

const lingQiColumns = [
  { columnName: '灵器代码', columnCode: 'lingQiCode', valueType: 'S', displayType: 'I', valueList: null, hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'N', editField: 'N', listField: 'Y', sortField: 'N' },
  { columnName: '灵器名称', columnCode: 'lingQiName', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: '110px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  // { columnName: '灵器分类', columnCode: 'lingQiFenLei', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: '90px', addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
  { columnName: '灵器描述', columnCode: 'lingQiMiaoShu', valueType: 'S', displayType: 'T', valueList: null, hiddenField: 'N', requiredFlag: 'N', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ lingQi, xiaoShuo, loading }) => ({
  lingQi,
  xiaoShuo,
  loading: loading.models.lingQi,
}))
export default class LingQi extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = lingQiColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      ...
      } else if (col.dataIndex === 'lingQiMiaoShu') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, lingQi: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'lingQi/query',
      payload: { ...pagination, ...params },
    });
    const { lingQi: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'lingQi/emptyProfile',
    });
  ...
  handleData = () => {
    const { lingQi: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { lingQi: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { lingQi: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑灵器"
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { lingQi: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { lingQi: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="灵器信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
