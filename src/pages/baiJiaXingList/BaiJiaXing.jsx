...
import styles from './BaiJiaXing.less';

const { Group: ButtonGroup } = Button;

const baiJiaXingColumns = [
  { columnName: '名称', columnCode: 'name', valueType: 'S', displayType: 'I', hiddenField: 'N', requiredFlag: 'Y', searchFlag: 'Y', profileField: 'Y', columnWidth: null, addField: 'Y', editField: 'Y', listField: 'Y', sortField: 'N' },
...
@connect(({ baiJiaXing, xiaoShuo, loading }) => ({
  baiJiaXing,
  xiaoShuo,
  loading: loading.models.baiJiaXing,
}))
export default class BaiJiaXing extends PureComponent {
  ...
  UNSAFE_componentWillMount() {
    this.listColumns = baiJiaXingColumns.filter(column => {
    ...
    this.listColumns = this.listColumns.map(col => {
      const colum = {};
      if (col.dataIndex === 'baiJiaXingCode') {
  ...
  async loadPlatServiceData(params = {}) {
    const { dispatch, baiJiaXing: { datas: { pagination } } } = this.props;
    this.setState({ loadingModel: 'list' });
    await dispatch({
      type: 'baiJiaXing/query',
      payload: { ...pagination, ...params },
    });
    const { baiJiaXing: { datas } } = this.props;
    // 先执行清空详情操作
    await dispatch({
      type: 'baiJiaXing/emptyProfile',
    });
  ...
  handleData = () => {
    const { baiJiaXing: { data }, xiaoShuo: { xiaoShuoList } } = this.props;
  ...
  renderButtonGroup() {
    ...
    if (currentModel === 'display') {
      const { baiJiaXing: { data: detailData, datas } } = this.props;
      if (detailData) {
  ...
  renderEditForm() {
    ...
    if (currentModel === 'edit') {
      const { baiJiaXing: { data: selectRecord  } } = this.props;
      return (
        <StandardForm
          title="编辑百家姓"
  ...
  renderEditDialog() {
    ...
    if (currentModel === 'listEdit') {
      const { baiJiaXing: { data  } } = this.props;
      return (
        <StandardForm
          formColumnList={this.editColumns}
          currentModel={currentModel}
  ...
  renderProfile() {
    const { currentModel } = this.state;
    if (currentModel === 'display') {
      const { baiJiaXing: { data: detailData } } = this.props;
      if (detailData) {
        return (
          <StandardProfile
  ...
  render() {
    const { baiJiaXing: { datas }, loading } = this.props;
    ...
    return (
      <PageHeaderWrapper>
        ...
        <Card title="百家姓信息" bordered={false} extra={this.renderButtonGroup()} loading={currentModel !== 'add' && loadingModel === 'profile' ? loading : false}>
          {this.renderProfile()}
          {this.renderEditForm()}
        </Card>
